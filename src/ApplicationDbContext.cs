...
public sealed class ApplicationDbContext(DbContextOptions<ApplicationDbContext> options) : IdentityDbContext<ApplicationUser, ApplicationRole, Guid>(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        _ = modelBuilder.Entity<ApplicationUser>().HasMany(u => u.SentDirectMessages).WithOne(m => m.Author).HasForeignKey(m => m.Author);
        ...
        _ = modelBuilder.Entity<ApplicationUser>().HasMany(u => u.SentGroupMessages).WithOne(m => m.Author).HasForeignKey(m => m.Author);
        _ = modelBuilder.Entity<ApplicationUser>().HasMany(u => u.SentIssueMessages).WithOne(m => m.Author).HasForeignKey(m => m.Author);
